  ...
  async handleLeaveRequest(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    try {
      ...
      if (request.status !== LeaveStatus.PENDING) {
        this.logger.log(
          `Leave request ${request.id} already processed. Status: ${request.status}`,
        );
      ...
      this.logger.log(
        `Leave request ${request.id} processed. New Status: ${newStatus}`,
      );
      channel.ack(originalMsg);
    } catch (error) {
      ...
      if (retryStrategy.shouldRetry(attempt, error)) {
        ...
        // MAX RETRIES REACHED
        this.logger.error(
          `Max retries reached for message, routing to DLQ. Payload: ${JSON.stringify(JSON.parse(originalMsg.content.toString()))}`,
        );

        // Nack without requeue: the queue's x-dead-letter-exchange
        // (see queue/topology.ts) routes it to leave_requests_dlq
        // for inspection instead of losing it.
