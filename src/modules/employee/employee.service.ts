...
export class EmployeeService {
  ...
  async findOne(id: string): Promise<Employee> {
    ...
    // Jitter the TTL (~1h +/- 5min) so keys warmed together (e.g. after a
    // deploy) don't all expire at once and stampede the DB.
    const ttlSeconds = 3600 + Math.floor(Math.random() * 300);
    await this.redis.set(cacheKey, JSON.stringify(employee), 'EX', ttlSeconds);
    return employee;
  }
