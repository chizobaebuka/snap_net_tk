...
export class CreateLeaveRequest1765228287205 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`leave_requests\` DROP FOREIGN KEY \`FK_leave_requests_employeeId\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_leave_requests_dates\` ON \`leave_requests\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_leave_requests_status\` ON \`leave_requests\``,
    );
    await queryRunner.query(
      `DROP INDEX \`IDX_leave_requests_employeeId\` ON \`leave_requests\``,
    );
